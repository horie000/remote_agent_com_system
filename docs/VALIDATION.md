...
- ホストPython 3.11.9: `python -m unittest discover -s tests -v` → 14件成功。
- UbuntuコンテナPython 3.10.12: 専用venvに `requirements.txt` を導入後、同じ14件が成功。
...
## 2026-09-24: turn/startの実CLI互換性

Ubuntu試験用Botの実コードから作成したturn/start要求をCodex CLI 0.156.1へ送り、ターンIDの返却を確認した。eadOnly.accessを含む旧形式はCLIが拒否したため、sandboxPolicy={type: readOnly}へ修正した。認証情報のない専用Codexホームで実施し、モデル応答とツール実行は確認していない。修正後のホスト・Ubuntu自動試験は各14件成功。
