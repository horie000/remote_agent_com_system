...
| 順序 | 作業 | 担当モデル | Gitの区切り |
| --- | --- | --- | --- |
| 1 | 仕様、承認境界、試験環境、実装計画 | 主担当Codex（現在のモデル） | docs: specification |
...
仕様を記録後に実装を開始する。意味のある区切りでcommit/pushし、force pushは使わない。秘密情報を除外する。認証がない場合もローカル試験まで実施し、実接続試験の未実施項目を明記する。pushは自動承認審査の外部送信先確認への回答を待つ。
